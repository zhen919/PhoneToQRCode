...
const CONFIG_KEY = "qr-code-config"
...
export default function HomePage() {
    ...
    useEffect(() => {
        ...
        if (config) {
            try {
                ...
                setUseLinkMode(parsed.useLinkMode ?? false)
            } catch (e) {
    ...
    useEffect(() => {
        localStorage.setItem(CONFIG_KEY, JSON.stringify({ useLinkMode }))
    }, [useLinkMode])
                ...
                <div className="mb-6 rounded-lg border border-border bg-card p-4">
                    <div className="flex items-center justify-between">
                        <div className="space-y-0.5">
                            <Label htmlFor="link-mode" className="text-sm font-medium">
                                使用跳转页面模式
                            </Label>
                            <p className="text-xs text-muted-foreground">
                                开启后，扫码将跳转到中间页面，点击按钮拨号（部分设备不支持直接拨号）
                            </p>
                        </div>
                        <Switch id="link-mode" checked={useLinkMode} onCheckedChange={setUseLinkMode} />
                    </div>
                </div>
